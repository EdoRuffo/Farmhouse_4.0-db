...
create table Nati (
    ...
    CodRiproduzione int,
    foreign key (CodAnimale) references Animale(CodAnimale)
);
...
create table StatoLocale (
	...
	foreign key (CodLocale) references Locale(CodLocale)
...
create table TipoArricchimento (
    ...
    primary key (TimeStamp),
	foreign key (CodAbbeveratoio) references Abbeveratoio(Codice)
);
...
create table StatoMangiatoia (
    ...
    primary key (TimeStamp),
    foreign key(CodMangiatoia) references Mangiatoia(Codice)
);
...
create table Pascolo (
    ...
    OrarioInizio time,
    Zona int,
    primary key (Data, OrarioRientro),
    foreign key (CodAnimale) references Animale(CodAnimale)
);
...
create table Delimitazione (
	...
	foreign key (NumeroRecinzione) references Recinzioni(Numero)
);
...
create table RilevamentoDisturbiComportamentali(
    ...
    primary key (Disturbo),
	foreign key (CodVisita) references Visita(CodVisita)
);
...
create table Assunzione(
    ...
    GiorniDiPausa int,
    primary key (Farmaco),
	foreign key (CodTerapia) references Terapia(CodTerapia)
);
...
create table SchedaDiGestazione(
    ...
    CodVeterinario int,
    foreign key (CodRiproduzione) references Riproduzione(Codice)
);
...
create table VisitaGravidanza(
    ...
    primary key (Data),
    foreign key (CodTerapiaG) references Terapiagravidanza(CodTerapiaG)
);
...
create table FasiLavorazioneIdeale(
    ...
    TempoDiRiposo int,
    primary key (nFase),
    foreign key (NomeFormaggio) references Formaggio(Nome)
);
...
create table ScaffalaturaCantina(
    ...
    CodCantina int NOT NULL,
    CodProdotto int,
    primary key (CodScaffalaturaC),
    foreign key (CodCantina) references Cantina(CodCantina)
);
...
create table Stagionature(
    ...
    foreign key (CodScaffalaturaC) references ScaffalaturaCantina(CodScaffalaturaC),
    foreign key (CodCantina) references Cantina(CodCantina)
);
...
create table PrenotazioneServiziAggiuntivi(
    ...
    DataFine date,
    foreign key (CodPrenotazione) references Prenotazione(CodPrenotazione),
    foreign key (NomeServizio) references ServiziAggiuntivi(NomeServizio)
);
...
create table PrenotazioneStanze(
    ...
    NumeroStanza int NOT NULL,
    foreign key (CodPrenotazione) references Prenotazione(CodPrenotazione),
    foreign key (NumeroStanza) references Stanza(NumeroStanza)
);
...
create table Itinerario(
    ...
    primary key (Area, DurataSosta),
    foreign key (CodEscursione) references Escursione(CodEscursione)
);
